...
- Node.js 20+（前端构建使用 Vite 8，需 20.19+ 或 22.12+；本地与 Docker 镜像已按此版本对齐）
- 本地或远程 MySQL 数据库
...
# 可选：前端地址（分享页「查看完整内容」跳转）
FRONTEND_URL=http://localhost:5173
# 可选：分享卡片链接根地址（生产环境建议设为 HTTPS 公网域名，如 https://bbs.example.com）
SHARE_BASE_URL=
```
...
复制帖子分享链接时，后端会生成 **`https://你的域名/s/帖子ID`**（带 Open Graph 的 H5 落地页），而不是 SPA 的 `/posts/:id`。微信爬虫不执行前端 JavaScript，若只分享 `/posts/:id` 往往只会显示纯链接。

**一般不需要与微信「商务合作」**，粘贴链接出现标题+摘要+缩略图卡片，只需：

1. 站点使用 **HTTPS 公网域名**（微信难以抓取纯 localhost / 内网）。
2. 配置 `SHARE_BASE_URL`（或与站点入口一致的 `API_BASE_URL`），并确保 Nginx 将 `/s/` 反代到后端（`web/nginx.conf` 已包含）。
3. 帖子有图时优先用首图；无图时使用站点 Logo 作为预览图。
4. 分享页 `/s/:id` **无需登录**即可被爬虫访问（当前仅展示已审核通过的帖子）。

若修改域名或样式后微信仍显示旧预览，可在 [微信公众平台](https://mp.weixin.qq.com/) → 设置 → 公众号设置 → 功能设置 中使用「分享链接查询/缓存刷新」工具（需已注册公众号）。

**仅在以下场景需要微信公众号 + JS-SDK 配置**（与粘贴链接预览不同）：用户在你的网页里打开后，点击微信右上角菜单自定义「发送给朋友 / 分享到朋友圈」的标题与图片。需要：认证服务号、JS 接口安全域名、后端签名接口等。
